...
test("docker image contains authoritative leave runtime modules", { timeout: 180000 }, (t) => {
  ...
  try {
    ...
    const fileCheck = run("docker", [
      ...
      "sh",
      "-lc",
      "test -f /app/ws-server/shared/poker-domain/leave.mjs && test -f /app/shared/poker-domain/leave.mjs && test -f /app/netlify/functions/_shared/poker-reducer.mjs && test -f /app/netlify/functions/_shared/supabase-admin.mjs && test -f /app/netlify/functions/_shared/api-cors.mjs && test -f /app/netlify/functions/_generated/deploy-context.mjs && test ! -f /app/netlify/functions/_shared/xp-cors.mjs"
    ]);
...
test("docker runtime resolves authoritative leave adapter default loader", { timeout: 180000 }, (t) => {
  ...
  try {
    ...
    const runtime = run("docker", [
      ...
      "sh",
      "-lc",
      "cd /app/ws-server && node --input-type=module -e \"import { createAuthoritativeLeaveExecutor } from './poker/persistence/authoritative-leave-adapter.mjs'; const execute = createAuthoritativeLeaveExecutor({ env: { SUPABASE_DB_URL: '' }, beginSql: async (fn) => fn({}), klog: () => {} }); const result = await execute({ tableId: 't1', userId: 'u1', requestId: 'r1' }); if (result?.code === 'temporarily_unavailable') { throw new Error('temporarily_unavailable'); } if (result?.ok === true) { throw new Error('unexpected_fabricated_success'); } if (typeof result?.code !== 'string' || result.code.length === 0) { throw new Error('unexpected_result_code'); }\""
    ]);
