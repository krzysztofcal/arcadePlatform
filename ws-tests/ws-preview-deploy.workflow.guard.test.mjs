...
test("ws preview deploy workflow packages and validates shared runtime files", () => {
  ...
  assert.match(text, /rsync -a --checksum --delete "\$TMP_EXTRACT_DIR\/ws-server\/" "\$PREVIEW_APP_DIR"\//);
  assert.match(text, /mkdir -p "\$PREVIEW_BASE_DIR\/shared"/);
  assert.match(text, /rsync -a --checksum --delete "\$TMP_EXTRACT_DIR\/shared\/" "\$PREVIEW_BASE_DIR\/shared"\//);
  assert.match(text, /mkdir -p "\$PREVIEW_BASE_DIR\/netlify\/functions\/_shared"/);
  assert.match(text, /rsync -a --checksum --delete "\$TMP_EXTRACT_DIR\/netlify\/functions\/_shared\/" "\$PREVIEW_BASE_DIR\/netlify\/functions\/_shared"\//);
  assert.match(text, /mkdir -p "\$PREVIEW_BASE_DIR\/netlify\/functions\/_generated"/);
  assert.match(text, /rsync -a --checksum --delete "\$TMP_EXTRACT_DIR\/netlify\/functions\/_generated\/" "\$PREVIEW_BASE_DIR\/netlify\/functions\/_generated"\//);
  ...
  assert.match(text, /rsync -a --checksum --delete "\$TMP_EXTRACT_DIR\/node_modules\/" "\$PREVIEW_BASE_DIR\/node_modules"\//);
