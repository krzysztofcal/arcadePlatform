(function () {
  ...
  function drawGameOver(){ ctx.save(); ctx.fillStyle='rgba(0,0,0,.5)'; ctx.fillRect(0,0,canvas.width,canvas.height); ctx.fillStyle='#f5f6fb'; ctx.font='24px "Courier New", monospace'; ctx.textAlign='center'; ctx.fillText('Game Over', canvas.width/2, canvas.height/2-10); ctx.font='16px "Courier New", monospace'; ctx.fillText('Press restart or jump to try again', canvas.width/2, canvas.height/2+14); ctx.restore(); }
  function spawnObstacle(){ const h=40+Math.random()*40,w=20+Math.random()*20; state.obstacles.push({x:canvas.width+Math.random()*60, y:GROUND_Y+2-h, width:w, height:h}); }
  function spawnCloud(){ state.clouds.push({ x:canvas.width+Math.random()*200, y:20+Math.random()*60, width:60+Math.random()*40, height:20+Math.random()*10, speed:30+Math.random()*20 }); }
  ...
  function render(){ ctx.clearRect(0,0,canvas.width,canvas.height); const grad=ctx.createLinearGradient(0,0,0,canvas.height); grad.addColorStop(0,'#15223c'); grad.addColorStop(1,'#0b0e19'); ctx.fillStyle=grad; ctx.fillRect(0,0,canvas.width,canvas.height); ctx.fillStyle='#23324d'; ctx.fillRect(0,GROUND_Y+10,canvas.width,3); ctx.fillStyle='#2f3e5f'; ctx.fillRect(0,GROUND_Y+13,canvas.width,2); ctx.fillStyle='rgba(255,255,255,.2)'; state.clouds.forEach(c=> drawRoundedRect(c.x,c.y,c.width,c.height,10)); ctx.fillStyle='rgba(0,0,0,.35)'; drawEllipse(state.dino.x+state.dino.width/2,GROUND_Y+12,26,6); drawDino(); ctx.fillStyle='#7cf58e'; state.obstacles.forEach(ob=> drawCactus(ob.x,ob.y,ob.width,ob.height)); }
