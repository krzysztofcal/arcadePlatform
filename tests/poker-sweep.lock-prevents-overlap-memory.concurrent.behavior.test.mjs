...
const run = async () => {
  ...
  const [first, second] = await Promise.all([handler(req), handler(req)]);
  const firstBody = JSON.parse(first.body);
  const secondBody = JSON.parse(second.body);

  const skippedCount = [firstBody, secondBody].filter((body) => body?.skipped === "locked").length;
  assert.equal(skippedCount, 1, "exactly one concurrent invocation should skip due to lock");
  assert.equal(workRuns, 1, "only one concurrent invocation should execute sweep work");
  assert.ok(logs.some((entry) => entry.event === "poker_sweep_skip_locked"));
  assert.equal(mem.has("poker:sweep:lock:v1"), false, "lock key should be released after run");
};
