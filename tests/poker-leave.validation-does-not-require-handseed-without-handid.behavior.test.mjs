...
const tableId = "14141414-1414-4141-8141-141414141414";
...
const handler = loadPokerHandler("netlify/functions/poker-leave.mjs", {
  ...
  isValidUuid: () => true,
  normalizeRequestId: () => ({ ok: true, value: "leave-no-handid-handseed" }),
  ensurePokerRequest: async () => ({ status: "proceed" }),
  storePokerRequestResult: async () => {},
  deletePokerRequest: async () => {},
  isStateStorageValid: (_state, opts = {}) => {
    if (opts.requireHandSeed || opts.requireCommunityDealt) return false;
    return true;
  },
...
const response = await handler({
  ...
  body: JSON.stringify({ tableId, requestId: "leave-no-handid-handseed", includeState: true }),
});

assert.equal(response.statusCode, 200);
const payload = JSON.parse(response.body || "{}");
assert.equal(payload.ok, true);
console.log("poker-leave validation does not require handseed without handid behavior test passed");
