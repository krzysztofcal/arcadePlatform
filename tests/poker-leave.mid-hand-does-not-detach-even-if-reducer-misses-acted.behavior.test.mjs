...
const handler = loadPokerHandler("netlify/functions/poker-leave.mjs", {
  ...
  isValidUuid: () => true,
  normalizeRequestId: () => ({ ok: true, value: "leave-mid-brittle" }),
  ...
  applyLeaveTable: (state) => ({
    state: {
      ...
      foldedByUserId: { ...(state.foldedByUserId || {}), [humanUserId]: false },
      actedThisRoundByUserId: { ...(state.actedThisRoundByUserId || {}), [humanUserId]: false },
      seats: state.seats,
      stacks: state.stacks,
      deck: state.deck,
    },
...
const response = await handler({
  ...
  body: JSON.stringify({ tableId, requestId: "leave-mid-brittle", includeState: true }),
});
...
console.log("poker-leave mid-hand does not detach when reducer misses acted behavior test passed");
