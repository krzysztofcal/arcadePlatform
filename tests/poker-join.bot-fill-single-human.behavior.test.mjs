      ...
      fn({
        unsafe: async (query, params) => {
          ...
          if (normalized.includes("insert into public.poker_seats") && normalized.includes("is_bot")) {
            const [, botUserId, seatNo, , stack] = params;
            if (seats.some((s) => s.user_id === botUserId || s.seat_no === seatNo)) return [];
            seats.push({ table_id: tableId, user_id: botUserId, seat_no: seatNo, status: "ACTIVE", is_bot: true, stack });
            botInsertCalls.push({ botUserId, seatNo });
          ...
          if (normalized.includes("coalesce(is_bot, false) = false")) {
            return [{ count: seats.filter((s) => s.status === "ACTIVE" && !s.is_bot).length }];
          }
          ...
          if (normalized.includes("select seat_no from public.poker_seats where table_id = $1 order by seat_no asc")) {
            return seats.map((s) => ({ seat_no: s.seat_no })).sort((a, b) => a.seat_no - b.seat_no);
          }
          if (text.includes("from public.chips_accounts")) return [{ id: "escrow-1" }];
          if (text.includes("from public.poker_state") && text.includes("for update")) return [{ version: 1, state: JSON.stringify({ tableId, seats: [], stacks: {}, phase: "INIT", pot: 0 }) }];
...
const run = async () => {
  ...
  assert.equal(ctx.seats.filter((s) => !s.is_bot && s.status === "ACTIVE").length, 1);
  assert.equal(ctx.seats.filter((s) => s.is_bot && s.status === "ACTIVE").length, 2);
  ...
  assert.equal(ctx.seats.filter((s) => s.is_bot).length, 2);
};
