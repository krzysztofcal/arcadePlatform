...
const stored = {
  ...
  state: {
    ...
    handSettlement: { handId: oldHandId, payouts: [{ userId: humanUserId, amount: 10 }] },
    seats: [
...
const run = async () => {
  const handler = loadPokerHandler("netlify/functions/poker-act.mjs", {
    ...
    applyAction: (state) => ({ state: { ...state, phase: "HAND_DONE", turnUserId: null, pot: 0 }, events: [{ type: "HAND_SETTLED" }] }),
  ...
  const response = await handler({
    ...
    body: JSON.stringify({ tableId, requestId: "auto-start-no-stale-settlement-1", action: { type: "CHECK" } }),
  });
...
run().then(() => console.log("poker-act auto-next-hand start-hand-core does not carry hand-settlement behavior test passed")).catch((error) => {
  console.error(error);
  process.exit(1);
});
