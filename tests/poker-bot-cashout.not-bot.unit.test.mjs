...
const run = async () => {
  ...
  const factory = new Function(
    ...
    `"use strict";
${rewritten}
return { ensureBotSeatInactiveForCashout };`
  );

  const postCalls = [];
  const { ensureBotSeatInactiveForCashout } = factory(
    async (payload) => {
  ...
  const result = await ensureBotSeatInactiveForCashout(
    {
      unsafe: async (query) => {
        const text = String(query).toLowerCase();
        if (text.includes("select user_id, seat_no, status, is_bot") && text.includes("is_bot = true") && text.includes("for update")) {
          return [];
        }
        return [];
      },
