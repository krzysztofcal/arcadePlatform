...
const stored = {
  ...
  state: {
    ...
    handId: "hand-leave-legal-shape",
    handSeed: "seed-leave-legal-shape",
    communityDealt: 0,
...
const run = async () => {
  const handler = loadPokerHandler("netlify/functions/poker-act.mjs", {
    ...
    applyAction,
    applyLeaveTable,
    computeLegalActions: () => ({ actions: ["CHECK", { type: "FOLD" }] }),
  ...
  const response = await handler({
    ...
    body: JSON.stringify({ tableId, requestId: "leave-legal-shape", action: { type: "LEAVE_TABLE" } }),
  });
...
run().then(() => console.log("poker-act leave ignores legal action shape behavior test passed")).catch((error) => {
  console.error(error);
  process.exit(1);
});
