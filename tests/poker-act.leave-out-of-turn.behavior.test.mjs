...
const stored = {
  ...
  state: {
    ...
    handId: "hand-leave-oot",
    handSeed: "seed-leave-oot",
    communityDealt: 0,
...
const run = async () => {
  ...
  const response = await handler({
    ...
    body: JSON.stringify({ tableId, requestId: "leave-oot", action: { type: "LEAVE_TABLE" } }),
  });
...
run().then(() => console.log("poker-act leave out-of-turn behavior test passed")).catch((error) => {
  console.error(error);
  process.exit(1);
});
