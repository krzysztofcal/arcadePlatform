...
const leaveRequestId = "leave-post-loop-idem";
...
const stored = {
  ...
  state: {
    tableId,
    phase: "TURN",
    handId: "hand-post-leave-idem",
    handSeed: "seed-post-leave-idem",
    communityDealt: 3,
...
const handler = loadPokerHandler("netlify/functions/poker-leave.mjs", {
  ...
  ensurePokerRequest: async (_tx, { requestId }) => {
    if (requestStore.has(requestId)) return { status: "stored", result: requestStore.get(requestId) };
...
console.log("poker-leave post-leave bot loop idempotent behavior test passed");
