...
const run = async () => {
  ...
  const response = await handler({
    ...
    body: JSON.stringify({ tableId, requestId: "legal-actions-req-1" }),
  });

  assert.equal(response.statusCode, 200);
  const payload = JSON.parse(response.body || "{}");
  assert.equal(payload.ok, true);
  assert.ok(Array.isArray(payload.legalActions), "response should always include legalActions");
  assert.equal(typeof payload.actionConstraints, "object", "response should include actionConstraints");
  assert.equal(payload.state?.state?.turnUserId, humanUserId, "requester should be UTG for this setup");
  assert.ok(payload.state?.state?.stacks?.[humanUserId] > 0, "requester should have stack available");
  assert.ok(payload.legalActions.length > 0, "requester should have at least one legal action when in turn");
};

run()
  .then(() => {
    console.log("poker-start-hand legal actions behavior test passed");
  })
  .catch((error) => {
    console.error(error);
    process.exit(1);
