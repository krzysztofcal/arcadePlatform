...
const stored = {
  ...
  state: {
    ...
    handId: "hand-leave-fold",
    handSeed: "seed-leave-fold",
    communityDealt: 0,
    ...
    leftTableByUserId: { [humanUserId]: false, [bot1UserId]: false, [bot2UserId]: false },
    sitOutByUserId: { [humanUserId]: false, [bot1UserId]: false, [bot2UserId]: false },
...
const run = async () => {
  ...
  const response = await handler({
    ...
    body: JSON.stringify({ tableId, requestId: "leave-fold", action: { type: "LEAVE_TABLE" } }),
  });
...
run().then(() => console.log("poker-act leave folds and advances behavior test passed")).catch((error) => {
  console.error(error);
  process.exit(1);
});
