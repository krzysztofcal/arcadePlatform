...
const makeHandler = ({ requestStore, postCalls, stateRef, helperCalls, queries }) =>
...
const run = async () => {
  ...
  const helperCalls = [];
  const queries = [];
  const handler = makeHandler({ requestStore, postCalls, stateRef, helperCalls, queries });
  ...
  assert.ok(statusUpdateIdx >= 0);
  assert.ok(stackUpdateIdx > statusUpdateIdx);
  ...
  const failingStateRef = {
    ...
    state: {
      tableId,
      seats: [{ userId: botUserId, seatNo: 2 }],
      stacks: { [botUserId]: 170 },
      pot: 0,
      phase: "INIT",
    },
  };
  const failingQueries = [];
  const failingHandler = loadPokerHandler("netlify/functions/poker-leave.mjs", {
    baseHeaders: () => ({}),
    corsHeaders: () => ({ "access-control-allow-origin": "https://example.test" }),
    extractBearerToken: () => "token",
    verifySupabaseJwt: async () => ({ valid: true, userId: botUserId }),
    ...
    cashoutBotSeatIfNeeded: async () => {
      throw new Error("cashout_failed");
    },
    beginSql: async (fn) =>
      fn({
        unsafe: async (query, params) => {
          const text = String(query).toLowerCase();
          failingQueries.push(text);
          if (text.includes("from public.poker_requests where")) return [];
          if (text.includes("insert into public.poker_requests")) return [{ request_id: params?.[2] }];
          if (text.includes("delete from public.poker_requests")) return [];
          if (text.includes("from public.poker_tables")) return [{ id: tableId, status: "OPEN" }];
          if (text.includes("from public.poker_state")) return [{ version: failingStateRef.version, state: JSON.stringify(failingStateRef.state) }];
          if (text.includes("from public.poker_seats") && text.includes("for update")) {
            return [{ seat_no: 2, status: "ACTIVE", stack: failingStateRef.seatStack, is_bot: true, user_id: botUserId }];
          }
          return [];
        },
      }),
  ...
  const failingResponse = await failingHandler({
    httpMethod: "POST",
    headers: { origin: "https://example.test", authorization: "Bearer token" },
    body: JSON.stringify({ tableId, requestId: "leave-fail-1" }),
  });
  assert.equal(failingResponse.statusCode, 500);
  assert.ok(!failingQueries.some((q) => q.includes("update public.poker_state set version = version + 1")));
  assert.ok(!failingQueries.some((q) => q.includes("delete from public.poker_seats")));
