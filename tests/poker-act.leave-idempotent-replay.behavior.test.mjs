...
const stored = {
  ...
  state: {
    ...
    handId: "hand-leave-replay",
    handSeed: "seed-leave-replay",
    communityDealt: 0,
...
const run = async () => {
  ...
  const event = {
    ...
    body: JSON.stringify({ tableId, requestId: "leave-replay", action: { type: "LEAVE_TABLE" } }),
  };
  ...
  const payloadFirst = JSON.parse(first.body || "{}");
  const payloadSecond = JSON.parse(second.body || "{}");
  assert.equal(payloadFirst.ok, true);
  assert.equal(payloadSecond.ok, true);
  assert.equal(payloadSecond.replayed, true);
  assert.equal(stored.stateUpdateCount, 1);
  assert.equal(stored.actionInsertCount, 1);
};

run().then(() => console.log("poker-act leave idempotent replay behavior test passed")).catch((error) => {
  console.error(error);
  process.exit(1);
});
