    ...
    fn({
      unsafe: async (query) => {
        ...
        if (text.includes("from public.poker_seats") && text.includes("status = 'active'") && text.includes("is_bot = false")) {
          return [{ user_id: "other", seat_no: 1 }];
        }
        if (text.includes("from public.poker_seats")) {
          return [{ user_id: "other", seat_no: 1, status: "ACTIVE", is_bot: false }];
        }
...
assert.equal(payload.me?.isSeated, false);

console.log("poker-get-table me notSeated db-shape behavior test passed");
