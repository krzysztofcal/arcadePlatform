...
function buildXpClientStub(config = {}) {
  const options = {
    ...
    omitStatusToday: !!config.omitStatusToday,
    omitStatusRemaining: !!config.omitStatusRemaining,
  };
    ...
    (function(){
      ...
      const stub = {
        ...
        fetchStatus(payload) {
          ...
          const snap = snapshot();
          snap.ok = true;
          if (config.omitStatusToday) delete snap.totalToday;
          if (config.omitStatusRemaining) delete snap.remaining;
          return Promise.resolve(snap);
        },
      };
...
test.describe('XP Progress page', () => {
  ...
  test('hydrates daily totals even when status omits the today field', async ({ page }) => {
    await clearStorage(page);
    await page.addInitScript({
      content: buildXpClientStub({
        cap: 500,
        award: 80,
        storageKey: '__xpProgressStatusFallback',
        omitStatusToday: true,
      }),
    });
    ...
    await page.evaluate(() => {
      try {
        window.localStorage.removeItem('kcswh:xp:last');
        window.localStorage.removeItem('kcswh:xp:regen');
      } catch (_) {}
    });
    ...
    const todayEarned = extractNumber(await page.textContent('#xpTodayLine'));
    expect(todayEarned).toBeGreaterThanOrEqual(totals.totalToday);
