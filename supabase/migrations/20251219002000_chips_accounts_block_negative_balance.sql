-- Hard invariant: chips_accounts.balance can never go below 0.
-- This protects you even if someone bypasses the ledger guard and inserts entries directly.
...
begin
  if new.balance < 0 then
    -- Use your canonical error so the API can map it to 400.
    perform public.raise_insufficient_funds();
  end if;
  return new;
end;
$$;
