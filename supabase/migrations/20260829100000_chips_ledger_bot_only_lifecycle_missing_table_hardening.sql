...
-- Forward-only replacement for the already-applied lifecycle receipt
-- hardening.  A missing TABLE row is legal only on an already-cleaned replay;
-- the first destructive cleanup must observe a persisted lifecycle marker in
-- the same transaction or roll back every prune, cleanup, and receipt write.
