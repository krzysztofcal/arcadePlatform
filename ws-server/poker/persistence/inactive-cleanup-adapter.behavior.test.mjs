...
test('inactive cleanup adapter uses module-relative shared loader urls and executes shared executor', async () => {
  ...
  const sharedDepsSrc = fs.readFileSync(new URL('../../../shared/poker-domain/inactive-cleanup-deps.mjs', import.meta.url), 'utf8');
  assert.match(sharedDepsSrc, /netlify\/functions\/_shared\/chips-ledger\.mjs/);
  assert.match(sharedDepsSrc, /netlify\/functions\/_shared\/poker-hole-cards-store\.mjs/);
