...
## Current V5 handoff — 2026-09-24

V5 continues the same draft PR #1016 from v4 HEAD `94c90f5a513626fe2cde0a560101a5e4f0baf31d`. Runtime commit `463313b4988a8b4ffa92b622bd6d26529703a704` extends the existing evaluated winner selection and card overlay to Straight, Flush, Full House, Four of a Kind, non-royal Straight Flush and Royal Flush. All six use the existing Royal card style, the existing owner-based large/compact sizes, and 1600 ms hero + 400 ms exit. Only each hand's title and authentic best five differ.

Live hand cards come from the current viewer's two cards or both revealed opponent cards plus the board. A public-board Royal Flush is sufficient proof without hole cards; other board-only hands fail closed. Confirmed positive contested-pot awards are required. Selection remains viewer-first among confirmed recipients and uses hand > Monster Pot > Win Streak in the single effect slot. The v4 compact `userId + seatNo` revalidation and explicit Preview FX target were preserved.

The existing PR-only Preview FX button remains gated by deploy-preview metadata. Its panel demonstrates all six hand categories in both sizes, with synthetic legal cards, and still offers Monster Pot and Win Streak (including selectable ×5/×6/×7/×8/×12). Browser smoke on the actual preview used a guest bot table at 390×844 and a short 320×480 viewport. All twelve hand demos rendered five cards, the common Royal style and 1600 ms hero on the selected bot; Monster Pot and Win Streak ×12 also rendered in both sizes. The button remained present; no page errors occurred. This is browser emulation, not physical Android acceptance.

Local validation on the V5 runtime commit: `node --test tests/poker-settlement-presentation.unit.test.mjs tests/poker-v2-live.behavior.test.mjs` **136/136 pass**; `PLAYWRIGHT_BROWSER=chromium npm run test:e2e` **66/66 pass**; JavaScript syntax, `git diff --check`, and CSP inline-hash guard pass (52 served documents). Only the existing fundamental settlement suite was extended; no UI/CSS/glue test suite was added.

Netlify Deploy Preview for runtime SHA `463313b4988a8b4ffa92b622bd6d26529703a704` succeeded as deploy `6ab55cc932ae24000837950d`. Served `BUILD_INFO` matched the full SHA and reported `context: deploy-preview`, `isPreview: true`, `pullRequest: true`, and `reviewId: 1016`. Canonical [PR Deploy](https://deploy-preview-1016--playkcswh.netlify.app/poker/). GitHub CI on this runtime SHA passed Tests, verify/Playwright, WS harness, actionlint, CodeQL, JavaScript/Actions analyses, games catalog, chips-retention and Netlify Deploy. Conditional chips-integration was skipped; the unrelated landing-site preview was canceled because that site had no changed pages.

The final PR description records the current exact PR HEAD and its matching Deploy Preview. Android acceptance of all six natural/preview animations remains with the owner. PR #1016 stays draft; no merge or Production deploy was performed. Main user-visible priority impact: verified hand art now occupies the single slot ahead of Monster Pot and Win Streak more often, while both effects' calculations and counters remain unchanged.
