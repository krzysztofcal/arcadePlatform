...
Local validation: `node --test tests/poker-settlement-presentation.unit.test.mjs tests/poker-v2-live.behavior.test.mjs` passed **135/135**; `PLAYWRIGHT_BROWSER=chromium npm run test:e2e` passed **66/66** on Chromium; JS syntax and the CSP inline-hash guard passed (52 served documents). GitHub CI for v4 runtime SHA `7fd1b5feb8f03a29a25ae89f43a1674ed4f8fa3f` passed `Tests`, `verify`/Playwright, `ws-harness`, `actionlint`, CodeQL, both analyzers, games catalog and chips-retention; conditional `chips-integration` was skipped. Existing project policy excludes new UI/CSS/glue tests.

Netlify's first exact-SHA preview attempt (`6ab52d78304fa300074e8e3e`) remained `building` with no error for more than 11 minutes; its build was canceled to retry the same PR-only deploy. That attempt never published and the PR alias still served v3 SHA `7245a610923b1ba8a84e42c8b76335971cc70623`. A new branch revision is needed to trigger the next automatic PR build; do not use the old alias for v4 acceptance. Final deployment metadata, browser preview smoke and physical Android status will be appended after the retry. The owner performs Android acceptance; the PR remains draft.
