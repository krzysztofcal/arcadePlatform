...
V4 runtime implementation is `7fd1b5feb8f03a29a25ae89f43a1674ed4f8fa3f`, based on the confirmed PR #1016 v3 HEAD `7245a610923b1ba8a84e42c8b76335971cc70623`. The latest successful automatic PR Deploy before this documentation-only evidence update used PR HEAD `9a44219b7076d413e0060219bdee354ed7c79395`; poker runtime files match the tested v4 implementation. This handoff edit changes documentation only. V4 preserves v3 result classification, Winner display, animation timing, settings and both visual sizes. Compact animation remains bound to one verified winner `userId + seatNo`; existing layout/render updates move it with that avatar through the ring-only tail. Changed identity clears immediately; temporarily unsafe geometry fades only the compact overlay without extending its original deadline. An invalid starting anchor skips the effect.
...
Local validation: `node --test tests/poker-settlement-presentation.unit.test.mjs tests/poker-v2-live.behavior.test.mjs` passed **135/135**; `PLAYWRIGHT_BROWSER=chromium npm run test:e2e` passed **66/66** on Chromium; JS syntax and the CSP inline-hash guard passed (52 served documents). GitHub CI on PR HEAD `9a44219b7076d413e0060219bdee354ed7c79395` passed Tests, verify/Playwright, ws-harness, actionlint, CodeQL, both analyzers, games catalog and chips-retention. Conditional chips-integration was skipped. Existing project policy excludes new UI/CSS/glue tests.

The first automatic Netlify runtime attempt (`6ab52d78304fa300074e8e3e`) stalled and was canceled without publishing. The retry `6ab5309b2c35ca000837a772` for documentation-only HEAD `66176b57e68cf7d6572043a479c8d78bc6fdf9c6` failed at `Install dependencies` because the command exceeded Netlify's time limit. A subsequent automatic PR build succeeded: deploy `6ab53828a1f0f40009e44aba`, current-at-that-time HEAD `9a44219b7076d413e0060219bdee354ed7c79395`, state `ready`, `context: deploy-preview`, `review_id: 1016`. Its served `BUILD_INFO` matches that SHA, `isPreview: true`, and `pullRequest: true`. The canonical [PR Deploy](https://deploy-preview-1016--playkcswh.netlify.app/poker/) now points to v4. A temporary manual branch alias was removed after the canonical preview recovered.
...
The final check set for PR HEAD `9a44219b7076d413e0060219bdee354ed7c79395` is green across GitHub CI and the automatic Netlify Deploy Preview. The earlier dependency-install timeout was transient and recovered on the next automatic build. The owner performs Android acceptance; PR #1016 remains draft. No merge or Production deployment was performed.
