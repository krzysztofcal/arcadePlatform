...
- [x] V4-04 Run the existing deterministic suites, Playwright, syntax/CSP and full PR CI; verify compact reflow/selection behavior and record the automatic Netlify PR Deploy failure plus the isolated v4 draft alias metadata; update the same draft PR body and Spec Kit evidence. Physical Android acceptance remains pending; do not merge or deploy Production.
...
V4 handoff is complete for implementation and browser verification. GitHub CI is green, but the automatic Netlify PR Deploy is not: its retry timed out while installing dependencies. The separate draft alias is documented in quickstart and is suitable for guest visual review; keep the PR draft until the automatic preview recovers and the owner accepts Android behavior.
