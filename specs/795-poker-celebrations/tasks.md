...
- [x] V4-04 Run the existing deterministic suites, Playwright, syntax/CSP and full PR CI; verify compact reflow/selection behavior; record the transient Netlify install timeout and subsequent successful exact-HEAD PR Deploy; update the same draft PR body and Spec Kit evidence. Physical Android acceptance remains pending; do not merge or deploy Production.
...
- [x] V5-04 Run the existing critical suites, syntax/CSP and Chromium checks; review diff; update `specs/795-poker-celebrations/quickstart.md` and the existing draft PR description with exact final SHA, matching Deploy Preview, CI state and Android handoff. Leave #1016 draft; do not merge or deploy Production.
