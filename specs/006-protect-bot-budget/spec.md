...
8. Zbiór ponad32 wpisów: pełna ocena registry znajduje zgodny cel także za32; output≤32 pasujące oferty. Brak pasującego celu w całym registry i pełny preflight mogą pozwolić≤1 create. Timeout/stale/proof failure nie jest completed empty. DIRECT nie zmienia sam celu.
...
- **FR-034**: D.3: ograniczyć pracę lobby/matchmaking/join/rollover/drain/close/refill/admin według jawnych limitów RT, rows, bytes, czasu, retry i concurrency w planie. Zero DB viewer×table/N+1 admin; brak częstego idle per-user pollingu; scalone refresh, bounded payload, discarded stale async. Complete no-match wymaga pełnej oceny całego aktualnego loaded/ready live registry WS oraz kompletnego sprawdzenia uprawnień i finansowania przed create po click. Payload lobby zawiera maksymalnie32 pasujące JOIN; nie ogranicza to zakresu wyszukiwania. Incomplete/stale/timeout/proof overflow oznacza unknown i zero create. Praca Supabase pozostaje bounded/coalesced, bez viewer×table SQL; niepełny dowód finansowy nie uprawnia mint. Refill low-priority, bounded i recoverable; ciężka opcjonalna praca pozostaje poza FIFO stołu, bez obietnicy wyprzedzania już zakolejkowanych komend; accepted settlement/cash-out nie zależą od tej pracy przy dostępnej DB, pełna awaria oznacza recovery bez fałszywej gwarancji. Egress mierzyć oddzielnie od CPU/Disk I/O/WAL/locks i wymagać baseline vs implementacja przed aktywacją. [D.3]
...
- **SC-007**: Fundamentalne scenariusze potwierdzają bounded/coalesced odczyty bez viewer×table, brak create przy failed/stale proof, pełne registry, także pasujący cel za32 wpisem bez ręcznej kontynuacji oraz poprawny settlement/recovery bez zależności od refillu. Przyszła bramka Q1/T038 mierzy RT/rows/bytes, CPU/I/O/WAL/locks i latency baseline vs implementacja; bez niezmierzonych SLO. [D.3]
