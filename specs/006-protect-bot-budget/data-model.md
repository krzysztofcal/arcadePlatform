...
W tym samym projektowanym journal dodać `MATCHMAKING_RECOMMENDATION` i `MATCHMAKING_RESULT`. Oba wymagają user_id; recommendation może mieć table_id=NULL wyłącznie dla terminalnego no-offer (bez create). Istniejące ograniczenia FUNDING/EXPOSURE bez zmiany; pola source/tier-funding/lineage/cost nie dotyczą tych faktów, nie tworzą allowance ani transferów. Rzeczywiste kolumny DB dla MATCH: user_id UUID, operation_id UUID, request_hash text, attempt_no smallint (1..2), search_step bigint (≥0 lub NULL); event_kind/event_key/result/table_id już z §3. operation_id/request_hash/attempt_no NOT NULL dla SEARCH_PROGRESS, MATCHMAKING_RECOMMENDATION i MATCHMAKING_RESULT; dla innych rodzajów nie dotyczą (NULL). search_step NOT NULL dla SEARCH_PROGRESS i HTTP MATCHMAKING_RECOMMENDATION (także no-offer), NULL dla MATCHMAKING_RESULT i innych rodzajów. `details` JSONB zawiera wyłącznie payload: requested_class/tier/maxPlayers/canonical stakes/intent, phase/last_checked_table_id dla progress, candidate_table_id/created_table_id/outcome/reason i descriptor cfg dla oferty; nie duplikuje kolumn kluczy. request_hash obejmuje stałe parametry konta/operacji; matched identity wyznacza serwer. Dwa indeksy częściowe, zamiast ogólnego UNIQUE wszystkich rodzajów: (A) UNIQUE(user_id,operation_id,event_kind,attempt_no) **wyłącznie WHERE event_kind IN (MATCHMAKING_RECOMMENDATION,MATCHMAKING_RESULT)**; (B) UNIQUE(user_id,operation_id,attempt_no,search_step) **wyłącznie WHERE event_kind IN (SEARCH_PROGRESS,MATCHMAKING_RECOMMENDATION)**. A dopuszcza najwyżej jedną końcową ofertę/no-offer i jeden wynik admission na próbę, nie obejmuje SEARCH_PROGRESS. B pozwala na wiele kroków progress, lecz najwyżej jedną odpowiedź (progress lub oferta/no-offer) danego kroku. Nie tworzyć nieograniczonego indeksem częściowym UNIQUE z A; samo dodanie B nie naprawia jego konfliktu. event_key nadal globalnie UNIQUE; indeks lookup po user/operation/attempt/step. Dla recommendation result COMMITTED oznacza utrwaloną ofertę, nie przyjęcie gracza; no-offer jest DENIED. MATCHMAKING_RESULT COMMITTED/DENIED oznacza ostateczny wynik admission danej próby, zapisywany atomowo z efektem join/odmową; brak wyniku to pending/unknown, nie pozwolenie na rematch. Dane rekomendacji i wyniku append-only, payload mismatch/replay nie zmieniają faktu.
...
SEARCH_PROGRESS wymaga user_id, table_id=NULL, result=COMMITTED (tylko postęp). Kolumny i oba partial UNIQUE z §3 są jedyną definicją kluczy; phase/last_checked_table_id są w details. search_step=0 to pierwsza odpowiedź, token event_id kroku n żąda dokładnie n+1. Pod account guard odczytać istniejącą odpowiedź następnego kroku po kolumnach, następnie ewentualnie ją zapisać; konflikt B oznacza odczyt zwycięskiego identycznego payload, nie ponowny skan/przesunięcie. Użyć istniejącego wzorca insert-on-conflict bez błędu transakcji; jeśli wystąpi wyjątek SQL/serialization, najpierw rollback i bounded recovery w zdrowej tx, nigdy SELECT w aborted tx. Rozbieżny request_hash/payload odrzucić. Deterministyczny event_key obejmuje user/operation/attempt/search_step jako tożsamość odpowiedzi; replay nie mutuje starego receipt. MATCHMAKING_RESULT nadal wiąże jedną ofertę danej próby i nie uczestniczy w B. Nie zwiększać admission attempt za progress. Źródło/lineage/cost/to_state_version nie dotyczą progress. Candidate keyset id ASC; PK i predykaty OPEN/class/buy_in/max_players ocenić przez EXPLAIN.

Fundamentalny scenariusz T006/T033: w tym samym user/operation/attempt=1 zapisać SEARCH_PROGRESS search_step=0 za id100 i search_step=1 za id200; oba INSERT muszą się udać pod rzeczywistymi constraintami DB. Równoczesny/retry token kroku0 odtwarza step1, nie zapisuje step2 i nie przesuwa kursora ponownie. Oferta step2 kończy wyszukanie; kolejny progress dla step2 narusza B, druga oferta tej próby narusza A. Bez nowej tabeli.
