...
## T042-U — minimalne uzupełnienie do review i osobnej zgody

**Nieautoryzowane, niewykonane.** Raport poprzedniej próby zaakceptowany dla HEAD4852cbfe1aa86ab2aab5354e74e7d91ec11b2272. Pierwotne6×3×4 zużyło72 sekwencje; ten plan nie powtarza macierzy ani nie przedłuża poprzedniej zgody. T042-U to część istniejącego T042, nie nowe zadanie implementacyjne. Najpierw poprawność; dopiero minimalne brakujące dane kosztowe. Nie wykonuje T043.

### A. Dowody poprawności i decyzje, które blokują

| Brak / decyzja Q1 | Bez DB? | Najmniejszy eksperyment i kryterium | Co pozostaje do T038 |
|---|---|---|---|
| C1 brak pasującego stołu: czy wynik jest completed-no-match, a nie unknown | Tak dla semantyki; SQL wymaga lokalnego potwierdzenia | Mały fixture8 stołów, wszystkie jawnie niezgodne: oba read-only warianty dają zero kwalifikujących IDs. Osobno pusty zbiór i missing proof w modelu pamięciowym; tylko kompletna, świeża ocena może oznaczać no-match, nigdy zgodę finansową/create. | Rzeczywisty create i final WS admission po implementacji; nie mierzymy CH. |
| C2 zgodny cel poza obecnym LIMIT50: czy pushdown ogranicza zbędne create | Częściowo; kolejność/oracle w pamięci, rzeczywisty SQL lokalnie | Dokładnie51 stołów, pierwsze50 z6 seat rows/5 ACTIVE, ostatni z miejscem, wszystkie z poprawnymi canonical stakes. Obecny SELECT plus odtworzona read-only walidacja pickSeatNo kontra batch: zapisać znalezione/pominięte IDs, nie tylko rows/time. Nie zwiększać LIMIT i nie skanować w pętli do skutku. Powtórzyć logiczny wariant ties/UUID bez DB. | Reprezentatywne duże inventory i dobór limitów; niewidoczny cel poza świadomie ograniczonym poprawnym zakresem pozostaje wcześniej zaakceptowaną możliwością, nie nowym zakazem create. |
| C3 live WS full/closed vs persisted OPEN/free; persisted-only cel | Tak |4 jawne pary snapshotów w pamięci: live-full, live-closed, persisted-only, boot-not-ready. Sprawdzić zdolność każdego wariantu dostarczenia potrzebnego faktu, źródło/version i dopuszczalny wynik; brak faktu oznaczyć unknown. Nie dopisywać nowego endpointu/autoryzacji. | Rzeczywiste opóźnienie persystencji i transport/readiness po wdrożeniu. Jeśli żaden obecny mechanizm nie dowodzi świeżości, to otwarta luka Q1, nie zaliczony test przyszłego mechanizmu. |
| C4 incomplete/stale/error: kiedy wolno uznać dobór za ukończony | Tak |6 deterministycznych wejść modelu: SQL error, timeout, obcięty batch, stara revision, brak capability, niepełny proof. Oczekiwany wynik unknown/unavailable, nigdy no-match/create; bez realnego timeoutu DB lub fault injection w środowisku. | Integracja timeout/recovery i concurrent final admission. |
| C5 kolejność humans/tier/UUID, canonical parametry oraz granice seat freshness | Tak dla oracle;2 odczyty SQL dla cutoff | Jedno zestawienie≤8 stołów obejmuje równe sort timestamps, różne maxPlayers/tiery i last_seen_at przed/na/po cutoff. Porównać wynik SQL z jawnymi oczekiwaniami i wskazać różnice obecnego kontraktu. Nie implementować docelowego matchera ani uznawać samego modelu za działającą funkcję. | UI kolejność i rzeczywisty auto-join ręcznie Preview. |

Oracle i expected IDs zapisać przed odczytami. Syntetyczne SLOW/class/drain/cfg jako wejścia modelu, bez dodawania schema #869. Model poprawności wskazuje wymagane fakty i kontrprzykłady; nie potwierdza istniejącej capability, której kod nie ma. Żaden przypadek nie wykonuje create, seat, transferu ani emisji. S1-A bez zmian.

### B. Koszty — tylko brak niezbędny do porównania

| Brak / decyzja Q1 | Minimalny zakres | Odroczenie |
|---|---|---|
| Bajty rzeczywistego wyniku, RT i work amplification SQL vs batch | W odczytach C1/C2/C5 policzyć rows i UTF-8 bytes tego samego formatu projekcji, round trips oraz explain rows/loops/buffers; identyczny zakres i pola przy porównywaniu. Przejście current→per-candidate walidacja odtworzyć w pamięci z jednym batch seats, koszt historycznego N+1 policzyć z kodu jako szacunek, nie zmierzony RT. | Supabase egress, sieć, p95 i większa skala do T038. |
| Broadcast1/10 widzów, concurrency2, CPU/I/O/WAL/lock pressure | Nie są potrzebne do rozstrzygnięcia źródła prawdy i kontrprzykładów C1–C4; nie dodawać eksperymentu teraz. Istniejący brak viewer×table SQL pozostaje wymaganiem. | T038 dla faktycznej implementacji, chroniąc settlement/cash-out; brak pomiaru nie jest dowodem braku contention. |
| Koszt nowego HTTP lub projekcji C | Brak implementacji do pomiaru; nie tworzyć endpointu/triggera na próbę. Jawna luka w porównaniu, jeśli wariant okaże się konieczny. | T043 może ocenić potrzebę na podstawie dowodów; koszt implementacji później T038. Jeśli koszt jest konieczny już do wyboru, Q1 pozostaje otwarte i wymaga odrębnego planu, nie domysłu. |

### C. Zamknięty budżet uzupełnienia

- Część bez DB: maks20 pojedynczych, deterministycznych ocen fixture/oracle (C1 empty/proof2,C2 order2,C3 cztery,C4 sześć,C5 trzy +1 rezerwy); bez powtórzeń czasowych, aplikacji, sieci i testów UI. Maks10min i10MiB własnych plików. Każdy rozjazd udokumentować; nie „naprawiać” kodu w eksperymencie.
- Lokalna część DB tylko dla C1/C2/C5:3 fixtures ×2 warianty (current/batch) = **6 sekwencji**, każda dokładnie2 odczyty: wynik oraz EXPLAIN ANALYZE BUFFERS tego samego SELECT. Raz, bez warm-up/replikacji; brak podstaw do wniosków o rozkładzie latencji. Maks12 mierzonych top-level SELECT, maks40 wszystkich wykonanych top-level SELECT z walidacją targetu/setup; osobno raportować podzapytania planu. Brak trzeciego wariantu registry SQL — poprzednia próba już pokazała koszt jego ograniczonego zakresu, a C3 sprawdza semantykę bez DB.
- Jednocześnie maks51 tables/51 states/306 seats/60 syntetycznych kont,1 połączenie,15min wall dla całego uzupełnienia,100MiB łącznie z własnym klastrem/WAL/raportem. Brak instalacji, nowego indeksu lub migracji repo. Wyłącznie własny prywatny lokalny PG/socket bez TCP/cloud i bez ingerencji w istniejące klastry, jak w poprzedniej próbie; własny minimalny DDL/fixtures i cleanup wymagają nowej zgody.
