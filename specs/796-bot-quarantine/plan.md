...
1. JOIN/funding najpierw czyta config FOR SHARE,następnie zdobywa table row,state row,membership. Gate na dodatnim funding czy nowym admission; nie na każdej akcji gry.
2. Ustalić pełny zestaw potrzebnych kont USER/source/ESCROW/GENESIS (GENESIS tylko przy możliwym refillu) i zablokować rosnąco po account id przed ich mutacjami; wszyscy ludzie jednym batch. Dostosować prelock w dotkniętych callers; oba adaptery nie gwarantują dziś order. Dla istniejących stołów gate nie blokuje USER przed table; Create ma wyjątek config→USER→INSERT własnego nowego stołu,bez lockowania istniejących tables. Terminal/leave serialized tym samym table; cross-table deadlock/serialization może mimo tego wystąpić z innymi operacjami: rollback i bounded existing retry, bez partial success.
...
## First-farmer Create — klasyfikacja przed INSERT

Authenticated createPokerTableWithState używa tego samego bot-access classifiera co final WS JOIN/funding,na authoritative USER balance pod lockiem w create transaction. Nie sam cached/durable status. Restriction i farmer-only table/INIT/escrow commit atomowo,unknown fail-closed. Managed null user zachowuje trusted ordinary path. Quick Seat create dziedziczy helper bez osobnego parsera. Klient nie ustawia is_farmer_only.

Config: bounded singleton poker_bot_access_config z data-model §5,shared loader/parser,classifier i FOR SHARE; zastępuje independent env threshold,żadnego nowego API. Dodatkowy1row read per relevant decision,bez pollingu. Kolejność locks config→table/state→accounts; Create config→USER→nowy nieopublikowany INSERT table/state. Aktualizacja config nie zdobywa table/USER locks. Migracja schema/default config nie mintuje CH i zachowuje wcześniejsze Stage/forward-only gates. Final JOIN recheck pozostaje; public discovery,settlement/cash-out bez detektora.
