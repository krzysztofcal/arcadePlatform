...
- Existing fundamental coverage is in `tests/public-profiles.behavior.test.mjs`, `ws-server/poker/table/table-manager.behavior.test.mjs`, `ws-server/server.behavior.test.mjs`, and `tests/account-page.test.mjs`; the runner already includes the relevant files.
