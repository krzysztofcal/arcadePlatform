...
- [X] T003 [US1] Implement the local DB-only Stage preflight in `scripts/ops/chips-ledger-missing-table-bot-retirement.mjs` using only `SUPABASE_STAGE_DB_URL`, canonical Stage project/system identity, `createPruneStore(sql).getIdentity()`, and `chips_assert_archive_prune_stage()`; reject Production target/credential variables, do not require or read `SUPABASE_STAGE_URL` or `SUPABASE_STAGE_SERVICE_ROLE_KEY`, use `klog`, and do not expand `validateStageEnvironment()` (FR-006, FR-016, SC-008)
- [X] T004 [US1] Implement bounded read-only `audit` inventory and candidate discovery in `scripts/ops/chips-ledger-missing-table-bot-retirement.mjs` over the full `chips_transaction_idempotency` registry, including mapped and unmapped identities and material hot identities; classify by key/version, transaction type, user/producer/ownership, replay contract, hot/archive/prune state, policy, age, and rows/day/bytes/day, then discover exact v1 missing-table whole-batch candidates from the existing archive mappings. Revalidate the already-performed 2026-09-11 classification, route incomplete/conflicting evidence to unknown/fail-closed, and emit aggregate `klog` metrics including 30-day and one-year projections without calling the new retirement function, any mutation, or a generic/persistent classifier (US1/AC1-3, FR-001, FR-002, FR-017, SC-001, SC-002, SC-007)
- [X] T005 [P] [US1] Create the classification and baseline section in `docs/issue-978-idempotency-retention-audit.md`, distinguishing the accepted research counts from fresh Stage measurements and documenting the four V1 families, protected classes, fail-closed evidence rules, and no-authorization meaning of historical estimates (US1/AC1-3, FR-002, FR-003, SC-001, SC-002)
...
**Checkpoint**: The implementation provides full read-only inventory/candidate discovery, strict post-T011 `p_execute = false` validation, and an explicit exact-batch execute path with an immutable matching receipt and no second economic effect. No Stage or Production canary is performed by these implementation tasks; a canary requires later independent review and explicit owner GO.
...
**Independent Test**: Verify that the audit/reporting capability accepts or emits clearly separated historical, current before-canary, and post-canary evidence fields; each identity class has rows/day and bytes/day plus 30-day and one-year projections, and any indefinite class names its blocking contract. Post-canary live values remain pending until a later owner-authorized Stage canary.

### Implementation for User Story 4

- [X] T014 [P] [US4] Extend the read-only report in `scripts/ops/chips-ledger-missing-table-bot-retirement.mjs` to calculate residual hot registry rows/day, bytes/day, 30-day capacity, and one-year capacity by identity class from historical and current before/after evidence, while keeping those evidence states separate and leaving unsupported/unknown classes fail-closed; accept recorded canary evidence as input only and perform no Stage mutation (US4/AC1-3, FR-001, FR-002, FR-017, SC-001, SC-002, SC-007)
- [X] T015 [US4] Create the post-canary residual-horizon and target-evidence section/template in `docs/issue-978-idempotency-retention-audit.md` with an exact format for target identity, batch IDs, registry counts and hashes, receipt status, residual rows/day and bytes/day by class, 30-day and one-year projections, balances/next-entry-sequence/ledger/provenance checks, retry result, and abort/blocking status; mark live values as pending and state that they are recorded only after later independent review and an owner-authorized Stage `GO <batch_id>` canary, without requiring, fabricating, or collecting those values during implementation and without Stage or Production mutation (US4/AC1-3, FR-016, FR-017, SC-007, SC-008)
...
- **User Story 2 (P1)**: Depends on the shared mechanisms in T001/T002 and the audit/preflight shape from T003/T004; its tests precede the migration/operator tasks, and strict per-batch validation waits for T011 and is integrated in T012.
- **User Story 3 (P1)**: Shares T010/T011 but has independent protected-class and effective-guard regression checks in T008/T009.
- **User Story 4 (P2)**: Depends on the audit output and report shape from User Stories 1 and 2; it provides read-only before/after reporting and a future-evidence template, and it does not authorize or perform a mutation.
...
2. Complete the DB-only preflight and full-registry audit/candidate discovery in T003-T005; defer strict per-batch function validation until T011/T012.
...
5. Stop for independent review before any Stage canary; a later exact Stage canary requires explicit owner GO, and no automatic drain or Production rollout is part of MVP.
