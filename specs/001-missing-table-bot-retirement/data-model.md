...
The existing `chips_ledger_archive_batches_cleanup_receipt_check` gains one narrowly scoped valid state for the v1/source-policy combination. Its all-null state and schema-v2 bot-only state remain valid. The existing `chips_guard_archive_batch_mutations()` continues to make proof, prune, and cleanup receipts write-once and separate; it gains only the matching v1 cleanup branch.
