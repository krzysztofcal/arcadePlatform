// Game page script extracted from HTML and wired to CONFIG
(function(){
  ...
  // === Game ===
  const DEFAULT_STATE = window.CONFIG.DEFAULT_STATE;
  ...
  function levelParams(lv){
    const base = window.CONFIG.LEVEL.baseFall;
    const per = window.CONFIG.LEVEL.fallPerLevel;
    const spawnEvery = Math.max(window.CONFIG.LEVEL.spawnEveryMin, window.CONFIG.LEVEL.spawnEveryBase - lv*4);
    let maxCats = 1;
    for (const rule of window.CONFIG.LEVEL.maxCatsAt){ if (lv >= rule.level) { maxCats = rule.cats; break; } }
