...
| `table_reaction` | `{ "seatNo": integer, "targetSeatNo"?: integer, "reactionKey": string }` | Ephemeral table event delivered to current table connections; `targetSeatNo` is generic presentation metadata, while only targeted `nice_hand` uses the congratulations effect. Server-generated bot reactions may additionally use contextual bot-only keys such as `hurry_up`, `you_are_bluffing`, `i_was_bluffing`, `lucky`, `congrats`, `not_this_time`, and `ambient_*`; these keys are not accepted from human `reaction_send`. Each bot has an independent four-second cooldown and at most one pending reaction, so different bots may emit concurrently. Reactions are not stored in `streamLog`, snapshots, Supabase, or reconnect replay. |
