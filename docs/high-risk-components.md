...
|---|---|---|---|---|
| Maze Muncher | `games-open/pacman/` | The previous unresolved runtime and stale external upstream/MIT attribution were removed from the active runtime path and replaced with first-party Arcade Hub / AI-assisted canvas code using original maze geometry and simple shapes. `public/games/pacman/LICENSE`, `about/licenses.html`, `play.html` and `docs/provenance-audit.md` identify the replacement as repository MIT code. | Active runtime keeps the `pacman` route and game id for compatibility, but no external maze-chase clone, third-party assets, external runtime code, remote scripts or remote assets are used. | **KEEP** |
