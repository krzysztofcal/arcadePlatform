...
- Contextual reactions:
  ...
  - The runtime may emit `you_are_bluffing` after a raise (20%), `i_was_bluffing` after a bot wins by normal folds (50%), `nice_bluff` after another player wins by normal folds (50% fallback), `nice_hand` for a shown winning three-of-a-kind or better (70%), `wow` for a bot payout of at least 20 big blinds, and either `well_played` or `congrats` after another win (one 50% roll).
  - One aggregate `lucky` classifier may target a winner (40%) when authoritative shown-hand rank vectors are unusually close, the same category is decided by a kicker, or a read-only turn-board comparison shows that the river changed the winner. Multiple matching signals still cause only one probability roll.
  - A bot may answer a successfully broadcast targeted `nice_hand` with `thanks` (50%), greet a newly joined human with `hello` or `good_luck` (60%), or emit bot-only `hurry_up` after observing 80% of an unchanged human turn window (50%).
  - Once per hand, one 20% ambient roll may select an available bot and one neutral table-talk message. Ambient state is process-local and cleared with table runtime resources.
  ...
  - `hurry_up`, `you_are_bluffing`, `i_was_bluffing`, `lucky`, `congrats`, and all `ambient_*` messages are bot-only keys. Humans retain the existing reaction menu, including `nice_bluff`.
