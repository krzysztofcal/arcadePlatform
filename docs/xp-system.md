...
- Signed XP sessions use the shared secret/fingerprint contract in `_shared/xp-server-session.mjs`. Awards fail closed when enforcement is enabled and the secret is invalid; Redis validation outages return temporary unavailability rather than invalidating a good browser token. Status reads remain outside the signed-session gate.
...
- Confirmed authenticated awards also update rebuildable Redis sorted-set projections for all-time, the current Warsaw XP day, and the current Warsaw ISO week inside the same Lua operation. Guests never enter these indexes. A positive anon-to-account conversion synchronizes all-time only. Bounded public ranking reads, a separate authenticated `me` read, and the localized leaderboard UI are active in production behind `XP_LEADERBOARD_ENABLED=1`. Owners can opt out in Settings without hiding their public profile.
