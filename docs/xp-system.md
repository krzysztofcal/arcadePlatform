...
- Confirmed authenticated awards also update dark, rebuildable Redis sorted-set projections for all-time, the current Warsaw XP day, and the current Warsaw ISO week inside the same Lua operation. Guests never enter these indexes. A positive anon-to-account conversion synchronizes all-time only. No public leaderboard endpoint or UI is active yet.
