...
- Non-game pages that render `#xpBadge` perform an initial authenticated status read. When a session is already present, identity-bound local XP cache is cleared before the read so a stale anonymous or optimistic value cannot be shown as the account total; game hosts keep their existing `GameXpBridge`/core lifecycle.
