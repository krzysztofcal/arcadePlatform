...
- Confirmed authenticated awards also update rebuildable Redis sorted-set projections for all-time, the current Warsaw XP day, and the current Warsaw ISO week inside the same Lua operation. Guests never enter these indexes. A positive anon-to-account conversion synchronizes all-time only. Bounded public ranking reads and a separate authenticated `me` read are enabled automatically on Deploy Previews; production remains gated by `XP_LEADERBOARD_ENABLED=1`, and no leaderboard UI is active yet.
