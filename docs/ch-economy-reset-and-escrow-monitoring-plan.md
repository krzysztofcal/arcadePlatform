...
Status: implementation prepared in PR #717. Runtime guards, manual SQL, the stage Network Restrictions operator script, runbook and read-only Admin/Ops monitoring are included, but no stage or production operation has been executed.
...
netlify/functions/poker-create-table.mjs::handler() oraz netlify/functions/poker-quick-seat.mjs::handler() używają createPokerTableWithState(). Po prerequisite obie funkcje respektują `CHIPS_ENABLED` przed auth i połączeniem z DB.
...
Ten prerequisite domyka aplikacyjną lukę. Historyczne Deploy Preview nadal mogą zawierać wcześniejszy kod, dlatego stage reset nie polega wyłącznie na aplikacyjnym guardzie.
...
Management API pozostaje dostępne po ograniczeniu Postgresa, dzięki czemu operator może wykonać restore nawet po pomyłce w CIDR. Dashboard Supabase jest awaryjną ścieżką odtworzenia dokładnej konfiguracji z chronionego recovery JSON.

Oficjalne odniesienia operacyjne:

- https://supabase.com/docs/guides/platform/network-restrictions;
- https://supabase.com/docs/reference/api/get-projects-ssl-enforcement-configuration;
...
Guard jest obroną dodatkową. Twardym warunkiem stage resetu pozostaje późniejsze ograniczenie stage Postgres i poolera do VPS, ponieważ stare Deploy Preview zachowują kod i environment z czasu swojego deploymentu.
...
3. Dla stage zatrzymać `ws-server-preview.service` przed zmianą Network Restrictions.
4. Wykonać network `preflight`, zachowując dokładną konfigurację w prywatnym recovery JSON.
5. Ograniczyć stage Postgres i pooler do IP VPS, poczekać na `applied` i potwierdzić połączenie `psql` z VPS.
...
3. Dla stage, przy nadal zatrzymanym WS, przywrócić dokładną konfigurację Network Restrictions z recovery JSON i poczekać na `applied`.
4. Przywrócić `CHIPS_ENABLED=1` dla kontekstu, który został przełączony na maintenance.
5. Utworzyć świeży deploy właściwego kontekstu i potwierdzić, że zawiera `CHIPS_ENABLED=1`; samo przestawienie ENV nie wystarcza.
6. Uruchomić odpowiedni `ws-server-preview.service` lub `ws-server.service` dopiero po sukcesie deployu.
7. Potwierdzić podstawowe operacje: odczyt salda, brak nieoczekiwanej mutacji, utworzenie nowego stołu i poprawne połączenie WS.

#### COMMIT przeszedł, ale końcowe assertions lub smoke zawiodły

1. Nie przywracać szerokiego dostępu do stage DB i nie uruchamiać WS.
...
5. Dopiero po poprawnym backup restore odtworzyć Network Restrictions, a następnie wykonać powyższą procedurę przywrócenia `CHIPS_ENABLED=1`, świeżego deployu i WS.
