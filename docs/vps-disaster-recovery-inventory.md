...
| Production WS env drop-in | `/etc/systemd/system/ws-server.service.d/override.conf` | Configuration that should be versioned | `infra/vps/ws-server.service.d/override.conf` | Install drop-in with `EnvironmentFile=-/etc/arcadeplatform/ws-server.env` |
