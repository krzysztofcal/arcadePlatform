...
- `calculate-xp.mjs` is the only gameplay XP writer and calculates grants from bounded semantic activity windows while enforcing session and Warsaw-local daily caps. `award-xp.mjs` is a temporary read-only status adapter; client delta requests return `410 legacy_award_retired`.
