...
- XP awards accept only catalog-backed browser game IDs. Structurally invalid award fields return `422`, unsupported games return `422 unsupported_game`, and request bodies above 16 KiB return `413 payload_too_large` before award-state mutation. Poker is an explicit exception because its XP does not use the browser award transport.
...
- Signed XP award sessions prefer the environment-scoped `XP_SESSION_SECRET` (minimum 32 characters) and temporarily fall back to `XP_DAILY_SECRET`. Deploy the renewal-capable client before introducing `XP_SESSION_SECRET`; existing fallback-signed tokens will renew once. Observe `invalid_session`, `start-session`, and session rate-limit telemetry before setting `XP_REQUIRE_SERVER_SESSION=1` and `XP_SERVER_SESSION_WARN_MODE=0`. Rollback changes only those enforcement flags and requires a fresh deploy; do not rotate the secret again during rollout or rollback.
