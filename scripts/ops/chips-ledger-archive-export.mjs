...
export const BOT_ONLY_CANDIDATE_NARROWED_TABLE_TRANSACTIONS_CTE = `
candidate_table_ids as materialized (
...
table_transaction_metadata as materialized (
  select transactions.*,
    ...
    from public.chips_transactions transactions
    join needed_registry_rows registry
...
with ${BOT_ONLY_CANDIDATE_NARROWED_TABLE_TRANSACTIONS_CTE}, table_rows as materialized (
  select registry.table_id,
         max(registry.transaction_created_at) as newest_created_at,
         count(*)::bigint as identity_count,
         count(*) filter (
