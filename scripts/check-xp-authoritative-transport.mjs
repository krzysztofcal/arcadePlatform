...
const legacyHandlerUrl = new URL("../netlify/functions/award-xp.mjs", import.meta.url);

assert.equal(client.includes('"/.netlify/functions/award-xp"'), false, "XPClient must not reference the retired award transport");
assert.equal(fs.existsSync(legacyHandlerUrl), false, "retired award-xp function must stay removed");
