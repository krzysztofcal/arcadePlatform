...
- Clicking the XP badge opens the XP Progress dashboard, which surfaces your current level, total XP, daily cap, XP earned today, remaining allowance, and basic boost/combo hints. The UI reads straight from the runtime via `XP.getSnapshot()`, `XP.getRemainingDaily()`, `XP.getNextResetEpoch()`, `XP.getBoostSnapshot()`, and `XP.getComboSnapshot()` so any page can render the same live totals.
