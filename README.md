...
- Once unlocked, the recorder auto-starts (`window.KLog.start(1)`) and the About page surfaces a **Dump diagnostics** button. Clicking it copies the recent buffer (up to 1000 lines) to the clipboard and falls back to downloading `kcswh-diagnostic-<timestamp>.txt` when clipboard access fails.
